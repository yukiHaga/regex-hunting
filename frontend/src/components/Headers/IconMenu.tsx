  ...
  ({
    ...
    const handleLogout = () => {
      deleteUserSession()
        ...
        .then(() => {
          navigate("/", {
            state: { display: true, success: "ログアウトしました。" },
            replace: true,
          });
        })
        .catch((e) => {
    ...
    return (
      <>
        <Box sx={{ flexGrow: 0, mx: 2 }}>
          <Tooltip title={user.name as string}>
            <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
              <Avatar alt="user-icon" src={user.image || DefaultAvatarImage} />
            </IconButton>
          </Tooltip>
          <Menu
            sx={{ mt: "45px" }}
          ...
          >
            ...
            <MenuItem
              ...
              onClick={handleCloseUserMenu}
            >
              <Typography textAlign="center">アカウント設定</Typography>
            </MenuItem>
