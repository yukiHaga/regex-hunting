    ...
    () => [
      {
        title: "中級編",
        sentence:
          "中級編を始める前に、最大量指定子(? * + {min,max})、キャプチャグループについて、スライドで学習しましょう！ そして、中級編のゲームを通して、これらの特殊文字を使用した正規表現をマスターしましょう！",
        slideNum: 0,
      },
      {
        title: "最大量指定子とは？",
        sentence:
          "最大量指定子(? * + {min,max})とは、ある文字列に繰り返しマッチさせるための特殊文字です。ある文字列の直後に最大量指定子を置くと、その文字列に対して最大量指定子を適用させることができます。正規表現で最大量指定子を使用することによって、繰り返しの文字列にマッチさせるのが簡単になります。以降のスライドでは、各最大量指定子の使用方法について説明します。",
        slideNum: 1,
      },
      {
        title: "?の使い方",
        sentence:
          "最大量指定子?は、直前の1文字があればマッチさせるが、なくてもよいという意味を表します。文字列の直後に?を置くことで、その文字列に?の意味が適用されます。例えば、n?はnがあればマッチさせるが、なくてもよいという意味を表します。n?を使用したenviron?mentという正規表現は、environment, enviromentという文字列にマッチさせることができます。このように、直前の1文字があってもなくてもよいという正規表現を作りたい場合に、?を使います。",
        slideNum: 2,
      },
      {
        title: "+と*の使い方",
        sentence:
          "最大量指定子+は、直前の1文字に1回以上の繰り返しマッチという意味を表し、最大量指定子*は、直前の1文字に0回以上の繰り返しマッチという意味を表します。+と*の意味は似ています。大きな違いとして、+の場合は少なくとも1回はマッチしないといけませんが、*の場合はマッチしなくても大丈夫です。以下の例のo+は、oの1回以上繰り返しマッチを表す為、cとkの間にoがない単語(ck)にはマッチしないことが分かります。直前の1文字を繰り返す正規表現を作りたい場合に、*や+を使います。",
        slideNum: 3,
      },
      {
        title: "{min,max}の使い方",
        sentence:
          "最大量指定子{min,max}は、直前の1文字にmin回以上、max回以上の繰り返しマッチという意味を表します。例えば、\\dは1桁の数字にマッチする為、\\d{3,5}は3桁以上、5桁以下の数字にマッチします。{min,max}以外にも、{min}や{min,}という書き方があります。{min}は直前の1文字にmin回繰り返しマッチを表し、{min,}は直前の1文字にmin回以上繰り返しマッチを表します。例えば、\\d{3}は3桁の数字、\\d{3,}は3桁以上の数字にマッチします。直前の1文字の繰り返し回数を制御したい場合に、{min,max}を使います。",
        slideNum: 4,
      },
      {
        title: "キャプチャグループとは？",
        sentence:
          "最大量指定子(? * + {min,max})は、どれも直前の1文字を対象とする特殊文字です。2文字以上の文字を繰り返したい場合はどうすれば良いのでしょうか？ 答えはキャプチャグループ((...))を使用することです。キャプチャグループを使用することで、複数の文字列を1つのグループとして扱うことができます。そのグループに最大量指定子を指定できる為、2文字以上の文字列を繰り返すことができます。本サービスでは、厳密性より見やすさと分かりやすさを重視している為、基本的には非キャプチャグループ((?:...))ではなく、キャプチャグループ((...))を使用します。",
        slideNum: 5,
      },
      {
        title: "ルール説明",
        sentence:
          "中級編では、最大量指定子、括弧を使用する正規表現を学習していきます。時間内に正しい正規表現を入力すると、モンスターに攻撃できます。モンスターのHPを0にしたらゲームクリアです！",
        slideNum: 6,
      },
    ],
    []
                    ...
                    <CodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <CodeLineWrapper>
                          ...
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/(メラ)+/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <ComentLineWrapper>
                          {"//"}{" "}
                  ...
                  <>
                    <CodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <BlankLineWrapper />
                        <ComentLineWrapper>
                          {"//"} environ?mentは、environment
                          enviromentにマッチする正規表現です。
                        </ComentLineWrapper>
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/environ?ment/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <ComentLineWrapper>
                          {"//"}{" "}
                    ...
                    <CodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <CodeLineWrapper>
                          ...
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/co*k/g</CodeYellowSpan>;{" "}
                          <CodeComentSpan>
                            {"//"} co*kは、ck cook coook
                            cooookにマッチする正規表現
                          </CodeComentSpan>
                        ...
                        <CodeLineWrapper>
                          ...
                          <CodeYellowSpan>/co+k/g</CodeYellowSpan>;{" "}
                          <CodeComentSpan>
                            {"//"} co+kは、cook coook cooookにマッチする正規表現
                          </CodeComentSpan>
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <CodeLineWrapper>
                    ...
                    <CaptureCodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex1{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/1(12)*/g</CodeYellowSpan>;{" "}
                          <CodeComentSpan>
                            {"//"}{" "}
                            (12)*は、12の0回以上の繰り返しマッチを表します。
                          </CodeComentSpan>
                        </CodeLineWrapper>
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex2{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/112*/g</CodeYellowSpan>;{" "}
                          <CodeComentSpan>
                            {"//"} 2*は、2の0回以上の繰り返しマッチを表します。
                          </CodeComentSpan>
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <CodeLineWrapper>
                  ...
                  >
                    <IconButton
                      sx={{
                        ...
                        cursor:
                          slideState.slideNum === 6 ? "default" : "pointer",
                      }}
