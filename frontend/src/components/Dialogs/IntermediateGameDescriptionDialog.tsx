...
export const IntermediateGameDescriptionDialog = ({
  ...
  const slideContent = useMemo(() => [
    ...
    {
      title: "+と*の使い方",
      sentence: "最大量指定子+は、直前の1文字に1回以上の繰り返しマッチという意味を表し、最大量指定子*は、直前の1文字に0回以上の繰り返しマッチという意味を表します。+と*の意味は似ています。大きな違いとして、+の場合は少なくとも1回はマッチしないといけませんが、*の場合はマッチしなくても大丈夫です。以下の例のo+は、oの1回以上繰り返しマッチを表す為、cとkの間にoがない単語(ck)にはマッチしないことが分かります。直前の1文字を繰り返す正規表現を作りたい場合に、*や+を使います。",
      slideNum: 3
    },
    {
      title: "{min,max}の使い方",
