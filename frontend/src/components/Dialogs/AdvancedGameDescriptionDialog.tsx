...
export const AdvancedGameDescriptionDialog = ({
  ...
  const slideContent = useMemo(() => [
    {
      title: "上級編",
      sentence: "上級編を始める前に、先読み、選択、アンカーについて、スライドで学習しましょう！ そして、上級編のゲームを通して、これらの特殊文字を使用した正規表現をマスターしましょう！",
      slideNum: 0
    },
    {
      title: "正規表現のマッチについて",
    ...
    },
    {
      ...
      sentence: "通常、マッチしたものが見つかった場合、それ以上マッチを試行するのをやめます。しかし、gオプションをつけている場合、マッチした文字列が見つかった後も、後ろの文字列に対して正規表現がマッチするかを試行します。そのため、Javaにもマッチします。以上が、ターゲット文字列に正規表現を適用させた時のマッチの流れです。実際の正規表現エンジンは最適化されている為、このようなマッチの流れと異なる動作をする場合があります。しかし、基本的にはこのようなマッチの流れなので、知っておくと正規表現を作るときに役立ちます。",
      slideNum: 2
    },
    {
      title: "肯定の先読みとは？",
      sentence: "(?=...)は、肯定の先読みと呼ばれます。肯定の先読みは、まだ試行していない右側の文字列に対して、...がマッチするかどうかを調べます。マッチする場合、肯定の先読みが成功し、...がマッチするかどうかの試行を開始した位置にマッチします。つまり、肯定の先読みは、...にマッチする文字列の位置を見つけます。重要なことは、全体の正規表現の最終的なマッチ結果に、肯定の先読みでマッチさせた文字列は含まれないということです。理由は、肯定の先読みのマッチ結果は位置を表すからです。肯定の先読みを使用することで、条件を絞ってマッチさせることができます。例えば、以下のコードのように、金額の数字のみにマッチさせることも可能になります。",
      slideNum: 3
    },
    {
      title: "否定の先読みとは？",
      sentence: "(?!...)は、否定の先読みと呼ばれます。肯定の先読みの=を!に変えると、否定の先読みになります。否定の先読みは、まだ試行していない右側の文字列に対して、...がマッチしないかどうかを調べます。マッチしない場合、否定の先読みが成功し、...がマッチしないかどうかの試行を開始した位置にマッチします。つまり、否定の先読みは、...にマッチしない文字列の位置を見つけます。否定の先読みは、ある文字列を含まないようなマッチをさせたい場合に使用します。",
      slideNum: 4
    },
    {
      title: "選択とは？",
    ...
    {
      title: "アンカーとは？",
      sentence: "アンカーは、文字ではなく行頭や行末等の決められた位置にマッチする特殊文字です。行頭は^、行末は$でマッチさせることができます。行頭や行末にマッチを固定したい場合に使用します。",
      slideNum: 6
    },
    {
      title: "ルール説明",
