...
export const AdvancedGameDescriptionDialog = ({
    ...
    () => [
      {
        title: "上級編",
        sentence:
          "上級編を始める前に、先読み、選択、アンカーについて、スライドで学習しましょう！ そして、上級編のゲームを通して、これらの特殊文字を使用した正規表現をマスターしましょう！",
        slideNum: 0,
      },
      {
        title: "正規表現のマッチについて",
        sentence:
          "今まで説明を省いていましたが、正規表現でどのように文字列をマッチさせるかをここで説明します。一般的に、ターゲット文字列の1文字1文字に対して、順番に正規表現がマッチするかを試行しています。例えば、Python JavaScript Javaというターゲット文字列に対して、Java(Script)?という正規表現を適用させる場合を考えます。まず正規表現のJがターゲット文字列のPにマッチするかを試行します。マッチしないので、次に、Pの隣のyに対して正規表現のJがマッチするかを試行します。マッチする文字列が見つかるまで、その試行を繰り返します。それを繰り返すと、JavaScriptのJで初めて正規表現のJがマッチします。Jがマッチした為、次は正規表現のaがJの隣の文字列にマッチするかを試行します。ここでもマッチが成功します。以上の流れが繰り返される為、JavaScriptにマッチします。",
        slideNum: 1,
      },
      {
        title: "正規表現のマッチについて(続き)",
        sentence:
          "通常、マッチしたものが見つかった場合、それ以上マッチを試行するのをやめます。しかし、gオプションをつけている場合、マッチした文字列が見つかった後も、後ろの文字列に対して正規表現がマッチするかを試行します。そのため、Javaにもマッチします。以上が、ターゲット文字列に正規表現を適用させた時のマッチの流れです。実際の正規表現エンジンは最適化されている為、このようなマッチの流れと異なる動作をする場合があります。しかし、基本的にはこのようなマッチの流れなので、知っておくと正規表現を作るときに役立ちます。",
        slideNum: 2,
      },
      {
        title: "肯定の先読みとは？",
        sentence:
          "(?=...)は、肯定の先読みと呼ばれます。肯定の先読みは、まだ試行していない右側の文字列に対して、...がマッチするかどうかを調べます。マッチする場合、肯定の先読みが成功し、...がマッチするかどうかの試行を開始した位置にマッチします。つまり、肯定の先読みは、...にマッチする文字列の位置を見つけます。重要なことは、全体の正規表現の最終的なマッチ結果に、肯定の先読みでマッチさせた文字列は含まれないということです。理由は、肯定の先読みのマッチ結果は位置を表すからです。肯定の先読みを使用することで、条件を絞ってマッチさせることができます。例えば、以下のコードのように、金額の数字のみにマッチさせることも可能になります。",
        slideNum: 3,
      },
      {
        title: "否定の先読みとは？",
        sentence:
          "(?!...)は、否定の先読みと呼ばれます。肯定の先読みの=を!に変えると、否定の先読みになります。否定の先読みは、まだ試行していない右側の文字列に対して、...がマッチしないかどうかを調べます。マッチしない場合、否定の先読みが成功し、...がマッチしないかどうかの試行を開始した位置にマッチします。つまり、否定の先読みは、...にマッチしない文字列の位置を見つけます。否定の先読みは、ある文字列を含まないようなマッチをさせたい場合に使用します。",
        slideNum: 4,
      },
      {
        title: "選択とは？",
        sentence:
          "|は、選択と呼ばれます。またはという意味を持つ特殊文字です。選択を使用することで、複数の正規表現のどれかにマッチするような1つの正規表現を作ることができます。選択を使用する上で注意すべきことは、選択の優先順位は非常に低いということです。例えば、This is React|JavaScriptという正規表現は、This is ReactまたはThis is JavaScriptにマッチする正規表現ではありません。React|JavaScriptで1つの塊に見えますが、選択の優先順位が非常に低い為、この正規表現はThis is ReactまたはJavaScriptにマッチする正規表現を表します。もし選択の範囲を限定したい場合、キャプチャグループを使用します。",
        slideNum: 5,
      },
      {
        title: "アンカーとは？",
        sentence:
          "アンカーは、文字ではなく行頭や行末等の決められた位置にマッチする特殊文字です。行頭は^、行末は$でマッチさせることができます。行頭や行末にマッチを固定したい場合に使用します。",
        slideNum: 6,
      },
      {
        title: "ルール説明",
        sentence:
          "上級編では、先読み、選択、アンカーを使用する正規表現を学習していきます。時間内に正しい正規表現を入力すると、モンスターに攻撃できます。モンスターのHPを0にしたらゲームクリアです！",
        slideNum: 7,
      },
    ],
    []
  ...
  const changeSlideToRight = useCallback(() => {
    if (slideState.slideNum < 7 && slideState.keyAndClickAvailable) {
      setSlideState((prev) => ({
        ...prev,
        slideIn: false,
        slideOut: true,
                  ...
                  <>
                    <CaptureCodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <BlankLineWrapper />
                        <ComentLineWrapper>
                          {"//"} Java(Script)?は、JavaScript,
                          Javaにマッチする正規表現です。
                        </ComentLineWrapper>
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/Java(Script)?/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                      </CodeBlockDiv>
                    </CaptureCodeBlockWrapper>
                  </>
                  ...
                  <>
                    ...
                    <CaptureCodeBlockWrapper>
                      <CustomCodeBlockDiv>
                        ...
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex1{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/Java(Script)?/</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex2{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/Java(Script)?/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <CodeLineWrapper>
                          ...
                          <CodeBlueSpan>match</CodeBlueSpan>(regex1));{" "}
                          <CodeComentSpan>
                            {"//"} ={">"} ['JavaScript','Script', index: 7,
                            input: 'Python JavaScript Java',groups: undefined]
                          </CodeComentSpan>
                        </CodeLineWrapper>
                        <CodeLineWrapper>
                          console.<CodeBlueSpan>log</CodeBlueSpan>(target.
                          <CodeBlueSpan>match</CodeBlueSpan>(regex2));{" "}
                          <CodeComentSpan>
                            {"//"} ={">"} ['JavaScript', 'Java']
                          </CodeComentSpan>
                        </CodeLineWrapper>
                    ...
                    <CaptureCodeBlockWrapper>
                      <CodeBlockDiv>
                        ...
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/\d{3}(?=yen)/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <CodeLineWrapper>
                          console.<CodeBlueSpan>log</CodeBlueSpan>(target.
                    ...
                    <CaptureCodeBlockWrapper>
                      <CodeBlockDiv>
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> target{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>'cat dog cat'</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <CodeLineWrapper>
                          <CodeRedSpan>const</CodeRedSpan> regex1{" "}
                          <CodeYellowSpan>=</CodeYellowSpan>{" "}
                          <CodeYellowSpan>/cat/g</CodeYellowSpan>;
                        </CodeLineWrapper>
                        <BlankLineWrapper />
                        <ComentLineWrapper>
                          {"//"}{" "}
                  ...
                  >
                    <IconButton
                      sx={{
                        ...
                        cursor:
                          slideState.slideNum === 7 ? "default" : "pointer",
                      }}
