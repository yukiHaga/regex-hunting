...
export const anchorsRows = [
  {
    id: 1,
    name: '^',
    data: '^は、行の先頭位置にマッチします。マッチを行頭に固定したい時に使用します。注意点は、^を文字クラス内で使用すると意味が変わるということです。文字クラス内の先頭に存在する場合、否定文字クラスを表します。文字クラス内の2文字目以降に存在する場合、ただの文字列として扱われます。',
    example: '先頭のcatという文字列にマッチさせたい場合'
  },
  {
    id: 2,
    name: '$',
    data: '$は、行の末尾の位置にマッチします。マッチを末尾に固定したい時に使用します。',
