...
export const lookAroundsRows = [
  {
    id: 1,
    name: '(?=...)',
    data: '(?=...)は、肯定の先読みと呼ばれます。通常、正規表現をターゲット文字列に適用させる場合、ターゲット文字列の先頭から1文字1文字順番にマッチするかどうかを試行します。肯定の先読みは、まだ試行していない右の方のテキストに対して...がマッチするかどうかを調べます。マッチする場合、肯定の先読みが成功し、...がマッチするかどうかの試行を開始した位置にマッチします。つまり、肯定の先読みは、...にマッチするテキストの位置を見つけます。重要なことは、全体の正規表現の最終的なマッチ結果に、肯定の先読みでマッチさせた文字列は含まれないということです。理由は、肯定の先読みのマッチ結果は位置を表すからです。肯定の先読みを使うことで、条件を絞ってマッチさせることができます。例えば、以下のコードのように、金額の数字のみにマッチさせることも可能になります。',
    example: 'ターゲット文字列がThis water(100ml) is 100yenの場合'
  },
  {
    id: 2,
    name: '(?!...)',
    data: '(?!...)は、否定の先読みと呼ばれます。肯定の先読みの=を!に変えると、否定の先読みになります。否定の先読みは、まだ試行していない右の方のテキストに対して、...がマッチしないかどうかを調べます。マッチしない場合、否定の先読みが成功し、...がマッチしないかどうかの試行を開始した位置にマッチします。つまり、否定の先読みは、...にマッチしないテキストの位置を見つけます。否定の先読みは、ある文字列を含まないようなマッチをさせたい時に使用します。',
