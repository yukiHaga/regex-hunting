Question.seed(
  ...
  {
    id: 34,
    sentence: '単体のcatという文字列にマッチする正規表現を入力せよ。',
  ...
  {
    id: 36,
    sentence: '真ん中のcatという文字列にマッチする正規表現を入力せよ。',
    target_sentence: 'cat cat dog',
    sample_answer: '(?!^)cat',
    hint: '否定の先読み, 文字列を使用して、正規表現を作ってみましょう。',
    commentary: '正規表現catをターゲット文字列に適用させると、先頭のcatにもマッチしてしまいます。そのため、否定の先読みを使用します。(?!^)は、ターゲット文字列が先頭位置(^)にマッチしないかを試行します。もしマッチしない場合、否定の先読みが成功し、マッチを試行開始した位置にマッチします。したがって、(?!^)catで真ん中のcatにマッチする正規表現を表します。',
    difficulty: 'advanced'
  },
