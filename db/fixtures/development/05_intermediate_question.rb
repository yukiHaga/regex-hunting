Question.seed(
  ...
  # 10問目
  {
    ...
    commentary: 'bookとbreakの共通な文字列はb, kです。共通ではない文字列はooとreaです。ooとreaは2文字以上の単語なので、選択を使うと「oo|rea」と表せます。したがって、「b(?:oo|rea)k」はbook, breakにマッチする正規表現を表します。ちなみに、括弧を使わない場合、「boo|reak」という正規表現になります。この正規表現は、boo, reakにマッチする為、bookとbreakにはマッチしません。気をつけるべきことは、選択の優先順位は非常に低いということです。括弧を使用することで、選択の範囲を限定できます。そのため、解答では括弧を使用しました。余談ですが、「b[a-z]+k」でマッチさせることもできます。',
    difficulty: 'intermediate'
  },
  ...
  # 11問目
  {
    ...
    sample_answer: 'co*k',
    hint: '文字列と量指定子(「*」)を使用して、正規表現を作ってみましょう。',
    commentary: 'ck, cook, coook, cooookの共通な文字列はc, kです。cとkの間のoの数が単語ごとに違います。任意の数のoは、「o*」で表すことができます。そのため、「co*k」でck, cook, coook, cooookにマッチする正規表現を表します。',
    difficulty: 'intermediate'
  },
  ...
  # 14問目
  {
    ...
    hint: '文字列, 括弧, 量指定子(「?」)を使用して、正規表現を作ってみましょう。',
    commentary: 'Javaという文字列と、JavaScriptという文字列の違いは、Scriptという文字列があるかないかです。Scriptという文字列があるまたはなしは、(?:Script)?で表すことができます。そのため、「Java(?:Script)?」でJava, JavaScriptにマッチするような正規表現を表します。もし括弧をつけない場合、正規表現は「JavaScript?」となります。これは、JavaScript, JavaScripにマッチするだけであり、Javaにはマッチしません。「?」の対象はあくまで直前の1文字であるためです。括弧を使うことで、1文字以上の文字列を「?」の対象にすることができます。',
    difficulty: 'intermediate'
  },
)
