Question.seed(
  ...
  # 3問目
  {
    ...
    commentary: '全ての文字列に共通していることは、-の前が3桁の数字, -の後が4桁の数字ということです。000-0000にマッチさせないようにするためには、否定の先読みを使います。否定の先読みを使用することで、ある文字列を含まないようなマッチができます。今回の場合、(?!0{3}-0{4})で文字列が000-0000ではないかを調べることができます。否定の先読みが成功した後、ターゲット文字列が\\d{3}-\\d{4}にマッチするかを試行します。したがって、(?!0{3}-0{4})\\d{3}-\\d{4}で171-0022, 101-0051にマッチする正規表現を表します。',
    difficulty: 'advanced'
  },
