Question.seed(
  ...
  # 4問目
  {
    ...
    commentary: '[-19]は-, 1, 9のどれか1文字にマッチします。[19-]または[-\d]でマッチさせることもできます。-を文字クラスの真ん中に配置([1-9])すると、1から9の1桁の数字にマッチする為、1, -, 9にマッチしません。',
    difficulty: 'elementary'
  },
  ...
  # 5問目
  {
    ...
    sample_answer: '[cd^]',
    hint: '文字クラスを使用して、正規表現を作ってみましょう。特に^の位置に気をつけましょう。',
    commentary: '[cd^]はc, d, ^のどれか1文字にマッチします。[c^d]または[\w^]でマッチさせることもできます。^を文字クラスの先頭に配置([^cd])するとc, d以外の1文字にマッチする為、^, c, dにマッチしません(否定文字クラス)。余談ですが、^は特殊文字であり、マッチを先頭に固定するという意味があります。しかし、文字クラス内で使用すると、否定文字クラスや普通の文字列にマッチする為、注意が必要です。',
    difficulty: 'elementary'
  },
  ...
  # 8問目
  {
    ...
    commentary: '[(\\\\*]は(, \\, *のどれか1文字にマッチします。\\をそのまま使用すると文字列として認識されません。そのため、\\を文字列として使いたい場合、\\で\\をエスケープします。',
    difficulty: 'elementary'
  },
