Question.seed(
  # 1問目
  {
    ...
    commentary: ' 「\\d」は1桁の数字を表します。「[0-9]」または「[159]」でマッチさせることもできます。',
  ...
  # 2問目
  {
    ...
    commentary: '「[a-z]」は1文字の小文字アルファベットを表します。「[acz]」または「\\w」でマッチさせることもできます。',
    difficulty: 'elementary'
  },
  ...
  # 3問目
  {
    ...
    sample_answer: '[bcf]',
    hint: '文字クラスを使用して、正規表現を作ってみましょう。',
    commentary: '「[bcf]」はb, c, fのどれか1文字を表します。「[a-z]」または「\\w」でマッチさせることもできます。',
    difficulty: 'elementary'
  },
  ...
  # 5問目
  {
    ...
    sample_answer: '[cd^]',
    hint: '文字クラスを使用して、正規表現を作ってみましょう。特に^の位置に気をつけましょう。',
    commentary: '「[cd^]」はc, d, ^のどれか1文字を表します。「[c^d]」または「[\w^]」でマッチさせることもできます。^を文字クラスの先頭に配置(「[^cd]」)するとc, d以外の1文字を表すので、^, c, dにマッチしません(否定文字クラス)。余談ですが、「^」は特殊文字であり、マッチを先頭に固定するという意味があります。しかし、文字クラス内で使うと、否定文字クラスや普通の文字列を表したりするので、注意が必要です。',
    difficulty: 'elementary'
  },
  ...
  # 11問目
  {
    ...
    hint: '文字クラス内で範囲を複数指定して、正規表現を作ってみましょう。',
    commentary: '「[6-8q-sQ-S.]」は6から8, ., アルファベットのどれか1文字を表します。余談ですが、.は任意の1文字を表す特殊文字です。しかし、文字クラス内で使用すると、エスケープしていなくても、ただの文字列として扱われます。',
    difficulty: 'elementary'
  },
